...
class WanAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm_cam = None 
        self.modulation_cam = None 
        self.cam_attn = None 
        self.projector = None 
        self.cam_encoder1 = None 
    ...
    def forward(
        ...
        with amp.autocast(dtype=torch.float32):
            if self.modulation_cam is not None:
                e1 = (self.modulation_cam + e[:, :3, :]).chunk(3, dim=1) # [B, 3, C]
            e = (self.modulation + e).chunk(6, dim=1)
        assert e[0].dtype == torch.float32

        # ~~param~~ recammaster
        cam_emb = None
        if self.cam_encoder is not None and camera_info_recam is not None:
            if self.transfer_layer is not None:
                cam_emb = self.transfer_layer(self.cam_encoder(camera_info_recam))
            else:
                cam_emb = self.cam_encoder(camera_info_recam)
            cam_emb = cam_emb * use_camera
        
        # pucker_param
        cam_emb1 = None
        if self.cam_encoder1 is not None and camera_info is not None: 
            cam_emb1 = self.cam_encoder1(camera_info)
            cam_emb1 = cam_emb1 * use_camera # 


        # self-attention
        if self.projector is not None and camera_info is not None:
            if cam_emb is not None and cam_emb1 is not None:
                y = self.projector(
                        self.self_attn(
                            self.norm1(x).float() * (1 + e[1]) + e[0] + cam_emb + cam_emb1, seq_lens, grid_sizes,
                            freqs, attn_mask
                        )
                    )
            elif cam_emb is not None:
                y = self.projector(
                        self.self_attn(
                            self.norm1(x).float() * (1 + e[1]) + e[0] + cam_emb , seq_lens, grid_sizes,
                            freqs, attn_mask
                        )
                    )
            else:
                y = self.projector(
                        self.self_attn(
                            self.norm1(x).float() * (1 + e[1]) + e[0] + cam_emb1, seq_lens, grid_sizes,
                            freqs, attn_mask
                        )
                    )
        else:
        ...
        if self.cam_attn is not None and camera_info is not None:
            # if use_camera is False:
            camera_info = camera_info * use_camera 
            z = self.cam_attn(
                self.norm_cam(x + camera_info).float() * (1 + e1[1]) + e1[0], seq_lens, grid_sizes,
                freqs, None) #
            with amp.autocast(dtype=torch.float32):
                x = x + z * e1[2]
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        _, h, w = x[0].shape[-3], x[0].shape[-2], x[0].shape[-1]
            ...
            if ii >= self.start_masked_layer and ii <= self.end_masked_layer:
                if self.training and use_gradient_checkpointing:
                    x = torch.utils.checkpoint.checkpoint(
                        ...
                        x, e0, seq_lens, grid_sizes, self.freqs, context, context_lens, premask, premask_text, camera_info, camera_info_recam, use_camera,
                        use_reentrant=False,
                    )
                else:
                    x = block(x, e0, seq_lens, grid_sizes, self.freqs, context, context_lens, premask, premask_text, camera_info, camera_info_recam, use_camera)
            else:
                if self.training and use_gradient_checkpointing:
                    x = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(block),
                        x, e0, seq_lens, grid_sizes, self.freqs, context, context_lens, None, None, camera_info, camera_info_recam, use_camera,
                        use_reentrant=False,
                    )
                else:
                    x = block(x, e0, seq_lens, grid_sizes, self.freqs, context, context_lens, None, None, camera_info, camera_info_recam, use_camera)
