...
class WanVace(WanT2V):
        ...
        for latent, refs in zip(latents, ref_images):
            if refs is not None:
                if masks is None:
                    ...
                    # print(f'ref_latent.shape:{ref_latent}',flush=True)
                else:
                    ref_latent = vae.encode(refs)
                    # print(f'ref_latent.shape:{ref_latent}',flush=True)
                    ref_latent = [
    ...
    def generate(self,
        ...
        if not self.t5_cpu:
            ...
            context = self.text_encoder(input_prompt, self.device) # wxx 更改为多个prompt
        ...
        target_shape = list(z0[0].shape) # 因为concat了ref_image 时间维度更长
        ...
        seq_len = math.ceil((target_shape[2] * target_shape[3]) /
                            (self.patch_size[1] * self.patch_size[2]) *
                            target_shape[1] / self.sp_size) * self.sp_size
        # print(f'noise.shape:{noise[0].shape}',flush=True)
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            klkl = len(input_ref_images[0])
            print(f'len(input_ref_images[0]):{klkl}')
            if not general_pr:
                if sum(mask_info) + len(input_ref_images[0]) != tt or len(mask_info) != text_mask:
                    raise ValueError("mask_info error!")
            else:
                if sum(mask_info) + len(input_ref_images[0]) != tt or len(mask_info) != text_mask - 1:
                    raise ValueError("mask_info error!")

            # p =   # 设置随机为True的概率，比如10%
            premask = (torch.rand(all_seq_length, all_seq_length, device=noise[0].device) < p)
            premask = premask.to(torch.bool)
            premask[:,:frame_tokens * (1 + len(input_ref_images[0]))] = True # 不止第一个token 还要加上前面的ref
            premask[:frame_tokens * (1 + len(input_ref_images[0])),:] = True # 不止第一个token 还要加上前面的ref
            ...
            # video-video
            temp_index = frame_tokens * len(input_ref_images[0])
            ...
            # camera
            # arg_camera = {'context': [context_null], 'seq_len': seq_len, 'premask': premask, 'premask_text': premask_text, 'camera_info':camera_info, 'camera_info_recam':camera_info_recam, 'use_camera':torch.tensor(1).to(self.device)}
            # text
